module LesliShield
    module AuthorizationInterface
        ...
        def authorize_request
            ...
            if granted.blank?
                current_user.journals.create({ title: "privilege_not_found", description: request.path })
                return respond_with_unauthorized({ controller: params[:controller], privilege: params[:action] })
            end

            unless granted
                current_user.journals.create({ title: "privilege_not_granted", description: request.path })
                return respond_with_unauthorized({ controller: params[:controller], privilege: params[:action] })
            end
        end
    end
